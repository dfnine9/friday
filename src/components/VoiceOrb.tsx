...
/**
 * Direct port of ethanplusai/jarvis orb.ts — the EXACT same visualization.
 * Floating particles with line connections. Lines fade based on state.
 * Speaking pulls particles closer for denser connections.
 * Electrons travel along connections during thinking.
 *
 * Adapted for React with useEffect lifecycle management.
 */
...
export default function VoiceOrb({ state, analyser, className }: VoiceOrbProps) {
  ...
  useEffect(() => {
    ...
    let transitionEnergy = 0, lastState: OrbState = "idle";
    ...
    function animate() {
      ...
      // State targets — exact same as Ethan's
      switch (currentState) {
        case "idle":
      ...
      if (currentState === "thinking") zTarget = Math.sin(t * 0.3) * 15 + Math.sin(t * 0.9) * 6;
      else if (currentState === "speaking") zTarget = Math.sin(t * 0.15) * 6 - bass * 10;
      ...
      if (currentState === "thinking") { mat.color.lerp(new THREE.Color(0x6ec4ff), 0.015); lineMat.color.lerp(new THREE.Color(0x6ec4ff), 0.015); }
      else if (currentState === "speaking") { mat.color.lerp(new THREE.Color(0x5ab8f0), 0.015); lineMat.color.lerp(new THREE.Color(0x5ab8f0), 0.015); }
