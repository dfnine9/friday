...
import { Zap, Terminal, Copy, Check, Play } from "lucide-react";
import { QUICK_ACTIONS } from "@/data/friday-data";

export default function ActionsSection() {
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const handleCopy = (cmd: string) => { navigator.clipboard.writeText(cmd); setCopiedId(cmd); setTimeout(() => setCopiedId(null), 2000); };
    ...
    <section id="actions" className="section">
      <div className="section-inner">
        <div className="section-header">
          <h2>Quick Actions</h2>
          <p>One-click access to 966 commands — launch skills, trigger agents, execute operations</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 stagger">
          {QUICK_ACTIONS.map((action) => (
            <div key={action.command} className="glass-card depth-card luminous-border p-5 group relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-white/[0.05]">
                  ...
                  <button onClick={() => handleCopy(action.command)} className="p-1.5 rounded-lg hover:bg-white/[0.04] transition-colors">
